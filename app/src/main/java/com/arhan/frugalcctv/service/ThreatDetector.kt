...
class ThreatDetector(
    ...
    companion object {
        ...
        private const val FACE_COOLDOWN = 2500L
    }
    ...
    fun onFrame(frame: VideoFrame) {
        ...
        try {
            ...
            if (now - lastMl >= ML_INTERVAL && busy.compareAndSet(false, true)) {
                ...
                val inputImage = InputImage.fromByteArray(bytes, ML_W, ML_H, frame.rotation, InputImage.IMAGE_FORMAT_NV21)

                faceDetector.process(inputImage)
                    .addOnSuccessListener { faces ->
                        if (faces.isNotEmpty() && now - lastFace >= FACE_COOLDOWN) {
                            lastFace = now
                            onEvent(SecurityEvent("HUMAN FACE DETECTED • ${faces.size} face(s) in view", Severity.CRITICAL))
                        }
                    }
                    .addOnCompleteListener {
                        labeler.process(inputImage)
                            ...
                            .addOnFailureListener { Log.w("FrugalCCTV", "ML Kit label detection failed", it) }
                            .addOnCompleteListener { busy.set(false) }
                    }
            }
        } catch (t: Throwable) {
            Log.e("FrugalCCTV", "Detection failed", t)
            busy.set(false)
        } finally {
            b.release()
        }
    ...
    private fun fastPath(b: VideoFrame.I420Buffer, now: Long) {
        val s = ByteArray(SW * SH)
        var sum = 0L
        var i = 0
        for (y in 0 until SH) {
            val sy = y * b.height / SH
            val row = b.dataY.position() + sy * b.strideY
            for (x in 0 until SW) {
                val sx = x * b.width / SW
                val v = b.dataY.get(row + sx).toInt() and 255
                s[i++] = v.toByte()
                sum += v
            }
        }
        val avg = sum.toDouble() / s.size
        val old = previous
        if (old != null) {
            var changed = 0
            var borderChanged = 0
            ...
            if (ratio >= 0.44 && borderRatio >= 0.40) {
    ...
    private fun toNv21(b: VideoFrame.I420Buffer, w: Int, h: Int): ByteArray {
        ...
        for (y in 0 until h / 2) {
            ...
            val ur = b.dataU.position() + sy * b.strideU
            val vr = b.dataV.position() + sy * b.strideV
            for (x in 0 until w / 2) {
                val sx = x * (b.width / 2) / (w / 2)
                out[d++] = b.dataV.get(vr + sx)
                out[d++] = b.dataU.get(ur + sx)
            }
        }
        return out
    }
