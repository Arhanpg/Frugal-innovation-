...
class MainActivity : ComponentActivity() {
    ...
    private fun AppRoot(service: CameraService?) {
        ...
        when (mode) {
            AppMode.HOME -> {
                HomeScreen(
                    camera = { mode = AppMode.CAMERA_SETUP },
                    viewer = { mode = AppMode.VIEWER_SETUP }
                )
            }

            AppMode.CAMERA_SETUP -> {
                SetupScreen(
                    title = "Camera setup",
                    subtitle = "Turn an old phone into your home security camera.",
                    room = room,
                    roomChange = { room = it },
                    url = url,
                    urlChange = { url = it },
                    ...
                    start = {
                        ...
                        requestCamera(room, url, key)
                        mode = AppMode.CAMERA
                    },
                    back = { mode = AppMode.HOME }
                )
            }
            ...
            AppMode.VIEWER_SETUP -> {
                SetupScreen(
                    title = "Viewer setup",
                    subtitle = "Connect to a camera using its room code.",
                    room = room,
                    roomChange = { room = it },
                    url = url,
                    urlChange = { url = it },
                    ...
                    start = {
                        service?.stopCamera()
                        prefs.saveConnection(url, key)
                        ensureNotificationPermission(this@MainActivity)
                        mode = AppMode.VIEWER
                    },
                    back = { mode = AppMode.HOME },
                    viewer = true
...
private fun AppTheme(content: @Composable () -> Unit) {
    MaterialTheme(
        colorScheme = androidx.compose.material3.darkColorScheme(
            ...
            surfaceVariant = Color(0xFF162A24)
        ),
        content = content
    )
}
...
private fun Header(title: String, subtitle: String, back: (() -> Unit)? = null) {
    Row(
        ...
        Column(modifier = Modifier.weight(1f)) {
            ...
            Text(subtitle, fontSize = 13.sp, color = Color(0xFF8FA59E))
        }
        Icon(
            Icons.Default.Shield,
            contentDescription = null,
            tint = Color(0xFF73E0B1),
...
@Composable
private fun HomeScreen(camera: () -> Unit, viewer: () -> Unit) {
    Column(
        modifier = Modifier
            .fillMaxSize()
        ...
        Text(
            "FRUGAL",
            color = Color(0xFF73E0B1),
            fontWeight = FontWeight.Bold,
            letterSpacing = 4.sp
        )
        Text(
            "Smart security,\nwithout buying a camera.",
            fontSize = 38.sp,
            lineHeight = 43.sp,
            fontWeight = FontWeight.Bold
        )
        ...
        Text(
            "Reuse an old Android phone as a real-time CCTV camera with WebRTC and on-device AI.",
            fontSize = 16.sp,
            color = Color(0xFF9CB0AA)
        )
        Spacer(Modifier.height(30.dp))
        ActionCard(
            "Camera mode",
            "Use this phone as the security camera",
        ...
        ActionCard(
            "Viewer mode",
            "Watch the live camera remotely",
...
private fun ActionCard(
    ...
    Card(
        ...
        colors = CardDefaults.cardColors(
            containerColor = if (primary) Color(0xFF123B2F) else Color(0xFF101F1B)
        )
    ) {
        Row(
            ...
            Icon(icon, contentDescription = null, tint = Color(0xFF73E0B1))
            Spacer(Modifier.size(16.dp))
            Column(modifier = Modifier.weight(1f)) {
                Text(title, fontSize = 19.sp, fontWeight = FontWeight.Bold)
                Text(desc, fontSize = 13.sp, color = Color(0xFF9CB0AA))
            }
            Icon(
                Icons.Default.ChevronRight,
                contentDescription = null,
                tint = Color(0xFF6E8780)
            )
        }
    }
}
    ...
    Column(
        ...
        Column(modifier = Modifier.padding(horizontal = 20.dp)) {
            ...
            OutlinedTextField(
                ...
                modifier = Modifier.fillMaxWidth(),
                label = { Text("Supabase publishable key") },
                singleLine = true
            )
            ...
            Button(
                ...
                Icon(
                    if (viewer) Icons.Default.Visibility else Icons.Default.PlayArrow,
                    contentDescription = null
                )
                Spacer(Modifier.size(8.dp))
                Text(
                    if (viewer) "Connect to camera" else "Start camera",
                    fontWeight = FontWeight.Bold
                )
            }
        }
...
@Composable
private fun CameraScreen(room: String, service: CameraService?, back: () -> Unit) {
    ...
    LaunchedEffect(service) {
        if (service != null) {
            armed = service.isArmed()
            audible = service.isAudible()
        }
    }
    LaunchedEffect(Unit) {
    ...
    Column(
        ...
        Row(
            ...
            Column(modifier = Modifier.weight(1f)) {
                Text("HOME CAMERA", fontWeight = FontWeight.Bold, color = Color.White)
                Text("Room $room", fontSize = 12.sp, color = Color(0xFF9FB2AC))
            }
            Pill(
                text = if (armed) "ARMED" else "DISARMED",
                tint = if (armed) Color(0xFF73E0B1) else Color(0xFFFFB86B)
            )
        }
        ...
        Box(
            ...
            AndroidView(
                ...
                update = { view -> service?.attachPreview(view) },
                modifier = Modifier.fillMaxSize()
            )
            if (service == null) {
                Text("Starting camera service…", color = Color.White)
            }
        }
        ...
        Row(
            ...
            FilledTonalButton(
                ...
                Text(if (audible) "Alarm on" else "Alarm off")
            }
        }
    }
}
...
private fun ViewerScreen(
    ...
    val ctx = LocalContext.current
    var status by remember { mutableStateOf("Connecting to signaling…") }
    ...
    Column(
        ...
        Row(
            ...
            Column(modifier = Modifier.weight(1f)) {
                Text("LIVE VIEW", fontWeight = FontWeight.Bold, color = Color.White)
                Text("Room $room", fontSize = 12.sp, color = Color(0xFF9FB2AC))
            }
            Pill(
                text = status.uppercase(Locale.US),
                tint = if (status == "Connected") Color(0xFF73E0B1) else Color(0xFFFFB86B)
            )
        }
        ...
        Box(
            modifier = Modifier
                ...
                .weight(1f)
                .padding(10.dp)
                .background(Color(0xFF0A100E), RoundedCornerShape(20.dp)),
            contentAlignment = Alignment.Center
        ) {
            AndroidView(
                factory = { SurfaceViewRenderer(it) },
                update = { view -> controller?.attachPreview(view) },
                modifier = Modifier.fillMaxSize()
            )
            ...
            alert?.let { message ->
                Text(
                    ...
                    modifier = Modifier
                        ...
                        .padding(18.dp)
                        .background(Color(0xCC7A321D), RoundedCornerShape(12.dp))
                        .padding(12.dp),
                    color = Color.White,
                    fontWeight = FontWeight.Bold
                )
        ...
        Text(
            text = if (armed) "Camera is armed — security alerts enabled" else "Camera is disarmed — tap Arm camera to enable alerts",
            modifier = Modifier.padding(horizontal = 16.dp, vertical = 5.dp),
            color = if (armed) Color(0xFF73E0B1) else Color(0xFFFFB86B),
            fontSize = 12.sp
        )
        ...
        FilledTonalButton(
            ...
            Text(if (armed) "Disarm camera" else "Arm camera")
        }
    }
}
        ...
        onIce = { candidate ->
            scope.launch {
                runCatching {
                    ...
                    postState("Signaling error: ${error.message ?: "ICE send failed"}")
                }
            }
        },
        onRemoteVideo = {},
        onConnection = { connection ->
            when (connection) {
                ...
                PeerConnection.IceConnectionState.CHECKING -> {
                    postState("Connecting video…")
                }

                PeerConnection.IceConnectionState.DISCONNECTED -> {
                    postState("Video connection interrupted")
                }

                PeerConnection.IceConnectionState.FAILED -> {
                    postState("Video connection failed")
                }

                PeerConnection.IceConnectionState.CLOSED -> {
                    postState("Viewer closed")
                }

                else -> {
                    postState("WebRTC: ${connection.name.lowercase().replace('_', ' ')}")
                }
            }
        },
        onError = { error ->
    ...
    fun start() {
        ...
        signaling.start(
            onMessage = { message ->
                when (message.type) {
                    "ready" -> {
                        if (negotiationStarted) return@startMessage
                            ...
                            rtc.createOffer { offer ->
                                scope.launch {
                                    runCatching {
                                        ...
                                        postState("Signaling error: ${error.message ?: "offer send failed"}")
                                    }
                                }
                            }
                        }.onFailure { error ->
                            negotiationStarted = false
                            postState("Error: ${error.message ?: "WebRTC offer failed"}")
                        }
                    }

                    "answer" -> {
                        val sdp = message.sdp ?: return@startMessage
                        activePeerId = message.from
                        rtc.setRemote(
                    ...
                    "ice" -> {
                        val candidate = message.candidate ?: return@startMessage
                        activePeerId = message.from
                        rtc.addIce(
                            IceCandidate(
                                message.sdpMid ?: "",
                    ...
                    "alert" -> {
                        message.text?.let(onAlert)
                    }

                    "arm" -> {
                        message.armed?.let(onArmed)
                    }
                }
            },
            onSubscribed = {
                ...
                scope.launch {
                    runCatching {
                        ...
                        postState("Signaling send failed: ${error.message ?: "hello failed"}")
                    }
                }
            },
            onError = { error ->
                postState("Signaling failed: $error")
            }
        )
    }

    fun attachPreview(view: SurfaceViewRenderer) {
        rtc.attachPreview(view)
    }
    ...
    fun setArmed(value: Boolean) {
        scope.launch {
            runCatching {
                ...
                postState("Signaling error: ${error.message ?: "arm command failed"}")
            }
        }
    }
...
private inline fun <T> returnAtStartMessage(block: () -> T): T? = block()
