...
class WebRtcSession(
    ...
    fun attachPreview(view: SurfaceViewRenderer) {
        preview?.let { old -> track?.removeSink(old); old.release() }
        preview = view
    ...
    private fun startCamera(context: Context) {
        ...
        val name = enumerator.deviceNames.firstOrNull { enumerator.isBackFacing(it) } ?: enumerator.deviceNames.firstOrNull()
        ...
        camera = enumerator.createCapturer(name, null) as? CameraVideoCapturer
            ?: throw IllegalStateException("Unable to create Android camera capturer")
    ...
    private fun observer() = object : PeerConnection.Observer {
        ...
        override fun onTrack(t: RtpTransceiver?) { (t?.receiver?.track() as? VideoTrack)?.let(onRemoteVideo) }
    ...
    fun createOffer(done: (SessionDescription) -> Unit) {
        peer?.createOffer(object : SdpObserverAdapter() { override fun onCreateSuccess(d: SessionDescription) { peer?.setLocalDescription(this, d); done(d) } }, MediaConstraints())
    }
    fun createAnswer(done: (SessionDescription) -> Unit) {
        peer?.createAnswer(object : SdpObserverAdapter() { override fun onCreateSuccess(d: SessionDescription) { peer?.setLocalDescription(this, d); done(d) } }, MediaConstraints())
    }
    fun setRemote(d: SessionDescription) {
        peer?.setRemoteDescription(object : SdpObserverAdapter() { override fun onSetSuccess() { remoteSet = true; queuedIce.forEach { peer?.addIceCandidate(it) }; queuedIce.clear() } }, d)
    }
    fun addIce(c: IceCandidate) { if (remoteSet) peer?.addIceCandidate(c) else queuedIce += c }
    fun release() {
        try { camera?.stopCapture() } catch (_: Exception) {}
        camera?.dispose(); helper?.dispose(); preview?.release(); track?.dispose(); source?.dispose(); peer?.dispose()
        preview = null; peer = null
    }
}
